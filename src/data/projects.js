/**
 * ZONECRAFT Project Portfolio Data
 * All 10 project links can be updated directly in this file.
 */

export const projects = [
  {
    id: 1,
    title: "🍔 Food Delivery Website",
    description:
      "Discover your favorite meals, order in seconds, and enjoy delicious food delivered straight to your doorstep.",
    category: "Business Website",
    technologies: [
      "React",
      "Tailwind CSS",
      "Vite",
      "express.js",
      "Nodejs",
      "MongoDb",
      "API",
    ],
    image: "/assets/services 1.jpeg",
    link: "https://food-del-ten-blue.vercel.app",
  },
  {
    id: 2,
    title: "✍️ Blog Website",
    description:
      "A modern space to share ideas, stories, knowledge, and experiences through engaging and beautifully presented content.",
    technologies: ["Next.js", "Tailwind CSS", "Framer Motion", "API"],
    image: "/assets/services 2.jpeg",
    link: "https://next-blog-site-rose.vercel.app/",
  },
  {
    id: 3,
    title: "🍽️ Restaurant Website",
    description:
      "Showcase delicious dishes, explore the menu, discover the ambiance, and make every dining experience memorable.",
    category: "Business  website",
    technologies: ["React", "Html", "CSS", "Lenis Scroll"],
    image: "/assets/services 3.jpeg",
    link: "https://xyz-resturant.vercel.app/",
  },
  {
    id: 4,
    title: "🏝️ Resort Website",
    description:
      "Escape to luxury and relaxation with stunning rooms, beautiful surroundings, premium amenities, and unforgettable experiences.",
    category: "Startup",
    technologies: ["React", "html", "CSS", "Lucide Icons", "Google-map-API"],
    image: "/assets/services 4.jpeg",
    link: "https://abc-resort.vercel.app/",
  },
  {
    id: 5,
    title: "💼 Portfolio Website",
    description:
      "A professional digital portfolio that highlights skills, projects, achievements, and creativity in a powerful way.",
    category: "Portfolio",
    technologies: ["React", "Tailwind CSS", "Vite", "Emailjs"],
    image: "/assets/services 5.jpeg",
    link: "https://yeshthakur.me/",
  },
  {
    id: 6,
    title: "☕ Cafe Website",
    description:
      "Experience the perfect blend of rich coffee, delicious bites, cozy vibes, and a warm café atmosphere.",
    category: "Ui/Ux",
    technologies: ["React", "Tailwind CSS", "WhatsAap-Integrated"],
    image: "/assets/services 6.jpeg",
    link: "https://cafe-hideout-five.vercel.app/",
  },
  {
    id: 7,
    title: "🎓 EduLearn Coaching & Academy Platform",
    description:
      "Digital presence for coaching institutes and educators with course schedules, mentor bios, and admission inquiries.",
    category: "Education",
    technologies: [
      "React",
      "Tailwind CSS",
      "Lucide Icons",
      "payment-integrated",
    ],
    image: "/assets/services 7.jpeg",
    link: "https://notes-app-alpha-opal.vercel.app/",
  },
  {
    id: 8,
    title: "🍬 Sweets Shop Website",
    description:
      "Explore a delightful collection of traditional and modern sweets, crafted with love to make every celebration sweeter.",
    category: "Landing Page",
    technologies: ["React", "Tailwind CSS", "Vite"],
    image: "/assets/services 8.jpeg",
    link: "",
  },
  {
    id: 9,
    title: "🛍️ E-Commerce Website",
    description:
      "A modern online shopping platform where customers can explore products, discover great deals, add items to their cart, and enjoy a seamless and secure shopping experience.",
    category: "Web Application",
    technologies: ["React", "Tailwind CSS", "REST API"],
    image: "/assets/services 9.jpeg",
    link: "",
  },
  {
    id: 10,
    title: "🤖 AI Thumbnail Generator",
    description:
      "Create eye-catching, professional thumbnails in seconds with AI-powered designs, customizable styles, colors, and layouts tailored for engaging content.",
    category: "UI/UX",
    technologies: [
      "React",
      "typescript",
      "open-AI-API",
      "MongoDB",
      "Nodejs",
      "Figma",
      "Tailwind CSS",
    ],
    image: "assets/services 10.jpeg",
    link: "",
  },
];

export const projectCategories = [
  "All",
  "Business Website",
  "Portfolio",
  "Landing Page",
  "E-Commerce",
  "Startup",
  "Education",
  "Web Application",
  "UI/UX",
];
